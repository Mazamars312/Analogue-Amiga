...
{
	HPS_spi_osd_cmd_cont(OSD_CMD_DISABLE);
	spi_w(0);
	spi_w(0);
	spi_w(0);
	spi_w(0);
...
void OsdMenuCtl(int en)
{
	if (en)
	{
		HPS_spi_osd_cmd(OSD_CMD_WRITE | 8);
		HPS_spi_osd_cmd(OSD_CMD_ENABLE);
	}
	else
	{
		HPS_spi_osd_cmd(OSD_CMD_DISABLE);
...
void OsdUpdate()
{
	...
	for (int i = 0; i < n; i++)
	{
		if (osdset & (1 << i))
		{
			HPS_spi_osd_cmd_cont(OSD_CMD_WRITE | i);
			HPS_spi_write(osdbuf + i * 256, 256, 0);
			HPS_DisableOsd();
